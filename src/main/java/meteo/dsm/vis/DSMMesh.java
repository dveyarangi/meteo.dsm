 ...
 * @author Xoppa */
public class DSMMesh implements Disposable {
	...
	public DSMMesh (boolean isStatic, int width, int height, boolean smooth, int attributes) {
		this(isStatic, width, height, smooth, MeshBuilder.createAttributes(attributes));
	}

	public DSMMesh (boolean isStatic, int width, int height, boolean smooth, VertexAttributes attributes) {
	...
	public void set(final DSMGrid grid )
	{
		this.grid = grid;
		update();
	}
	...
	public Vector3 getPositionAt (Vector3 out, int x, int y) {
		out.set(grid.getVectors().at(x, y).getCartesianPos());
		return out;
	}
...
// A more accurate approach would be to weight the normal of the actual triangles.
		...
		return out.set(out.x, out.y, out.z);
	}
